...
ActiveAdmin.register InfoCategory do
  index do
    column 'ID', :id
    column '名称', :title
  ...
  show do
    attributes_table do
      ...
      row "创建时间：" do
        info_category.created_at.strftime('%Y-%m-%d %H:%M:%S')
      end
      row "更新时间：" do
        info_category.updated_at.strftime('%Y-%m-%d %H:%M:%S')
      end
    end
  end
